...
void task_socket_srv( void *tp_arg )
{
	...
	TaskHandle_t l_rgb_task_handle = xTaskGetHandle("LED_RGB");
	// go!
	for( ;; )
	{
		...
		for ( l_reply_count = 0; pdTRUE; l_reply_count++ )
		{
			...
			if( l_len > 0 )
			{
				...
				if (strncmp(l_rx_buf, "LED #", 5) == 0 && strlen(l_rx_buf + 5) >= 2)
				{
					hex_to_int(l_rx_buf+5);
					PRINTF("recv hex led %s\n", l_rx_buf+5);
					xSemaphoreGive(g_sem_led);
				}

				if (strncmp(l_rx_buf, "RGB #", 5) == 0 && strlen(l_rx_buf + 5) >= 6)
				{
					if (l_rgb_task_handle)
					{
						vTaskSuspend(l_rgb_task_handle);
						hex_to_int_rgb(l_rx_buf + 5);
						vTaskResume(l_rgb_task_handle);
					}
				}

				if (strncmp(l_rx_buf, "LIST", 4) == 0)
				{
					char task_buff[2048] = {};
					vTaskList( task_buff );
					int len = 0;
					...
					{
						char out_buf[256] = {};
						if (strlen(task_buff+len) < 1 || len > 2048) break;
						strncpy(out_buf, task_buff+len, 256);
						len += 256;
						FreeRTOS_send( l_sock_client, ( void * ) out_buf, strlen(out_buf), 0 );
					}
				}
...
void task_led(void* arg)
{
	...
	while (1)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			GPIO_PinWrite( g_led_ptc[ i ].m_led_gpio, g_led_ptc[ i ].m_led_pin, g_led_data & 1 << (7-i) );
		}
		last_status = g_led_data;
	}
}

LED_Data g_buttons[4] =
{
...
{
	int last_status[4] = {1,1,1,1};
	while (1)
	{
		bool changed = false;
		for (int i = 0; i < 4; i++)
		{
			int st = GPIO_PinRead( g_buttons[i].m_led_gpio, g_buttons[i].m_led_pin );
			if (st != last_status[i])
			{
				last_status[i] = st;
				changed = true;
		...
		if (changed)
		{
			if (l_sock_client != NULL)
			{
				...
				{
					char c = '0';
					if (last_status[i] == 0) c = '1';
					buf[i+4] = c;
				}
				buf[8] = '\n';
				buf[9] = 0;
				PRINTF("send button change\n");
				FreeRTOS_send( l_sock_client, ( void * ) buf, strlen(buf), 0 );
			}
		}
		vTaskDelay(100);
	}
...
void task_rgb(void* arg)
{
	while(1)
	{
		// xSemaphoreTake(g_sem_rgb, portMAX_DELAY);
		for (int i = 0; i < 3; i++)
		{
			LED_RGB_Data rgb_led = g_led_rgb[i];
			if (ticks < g_rgb_data[0]/10) GPIO_PinWrite( rgb_led.r.m_led_gpio, rgb_led.r.m_led_pin, 1 );
			else GPIO_PinWrite( rgb_led.r.m_led_gpio, rgb_led.r.m_led_pin, 0 );

			if (ticks < g_rgb_data[1]/10) GPIO_PinWrite( rgb_led.g.m_led_gpio, rgb_led.g.m_led_pin, 1 );
			else GPIO_PinWrite( rgb_led.g.m_led_gpio, rgb_led.g.m_led_pin, 0 );

			if (ticks < g_rgb_data[2]/10) GPIO_PinWrite( rgb_led.b.m_led_gpio, rgb_led.b.m_led_pin, 1 );
			else GPIO_PinWrite( rgb_led.b.m_led_gpio, rgb_led.b.m_led_pin, 0 );
		}
...
int main(void) {
    ...
    //
	uint8_t ucMAC[ ipMAC_ADDRESS_LENGTH_BYTES ] = { 0x5A, 0xFE, 0xC0, 0xDE, 142, 93 };
	...
	if ( xTaskCreate( task_led, "LED_TASK", configMINIMAL_STACK_SIZE + 100, NULL, NORMAL_TASK_PRIORITY, NULL ) != pdPASS )
	{
		PRINTF( "Unable to create task '%s'!\r\n", "LED_TASK" );
	}

	if ( xTaskCreate( task_btn, "BTN_TASK", configMINIMAL_STACK_SIZE + 100, NULL, NORMAL_TASK_PRIORITY, NULL ) != pdPASS )
	{
		PRINTF( "Unable to create task '%s'!\r\n", "BTN_TASK" );
	}

	if ( xTaskCreate( task_rgb, "LED_RGB", configMINIMAL_STACK_SIZE + 100, NULL, NORMAL_TASK_PRIORITY, NULL ) != pdPASS )
	{
		PRINTF( "Unable to create task '%s'!\r\n", "LED_RGB" );
	}
