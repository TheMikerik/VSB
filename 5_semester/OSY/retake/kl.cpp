...
//				PRINTF( "Server forwarded %d bytes.\r\n", l_len );
				...
				char command [4];
				memset(command, 0, 4);
				strncpy(command, (const char*) l_rx_buf, 3);
				command[3] = '\0';

				if (strcmp(command, "ON ") == 0) {
					//turn on
					int index = l_rx_buf[3] - '0';
					if (index >= 0 && index <= 7) {
						GPIO_PinWrite( g_led_ptc[ index ].m_led_gpio, g_led_ptc[ index ].m_led_pin, 1 );
					}

				} else if (strcmp(command, "OFF") == 0) {
					//turn off
					int index = l_rx_buf[4] - '0';
					if (index >= 0 && index <= 7) {
						GPIO_PinWrite( g_led_ptc[ index ].m_led_gpio, g_led_ptc[ index ].m_led_pin, 0 );
					}

				}

				char command_2 [11];
				memset(command_2, 0, 11);
				strncpy(command_2, (const char*) l_rx_buf, 10);
				command_2[10] = '\0';

				if (strcmp(command_2, "LED ALL ON") == 0) {
					for (int i = 0; i < 8; i++) {
				...
				char command_3 [12];
				memset(command_3, 0, 12);
				strncpy(command_3, (const char*) l_rx_buf, 11);
				command_3[11] = '\0';

				if (strcmp(command_3, "LED ALL OFF") == 0) {
					for (int i = 0; i < 8; i++) {
