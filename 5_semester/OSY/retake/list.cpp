...
#define LED_COUNT 8 // Počet LED - 8 bitů
bool ptc_bool0[LED_COUNT] = {false};


// Funkce: Zpracování binárního čísla a mapování do LED
void process_leds(int number) {
    PRINTF("Číslo pro LED: %d\n", number);
    for (int i = 0; i < LED_COUNT; i++) {
        ptc_bool0[i] = (number & (1 << i)) ? true : false; // Mapování jednotlivých bitů na LED
        PRINTF("LED[%d] = %d\n", i, ptc_bool[i]); // Debugovací zprávy
        if (ptc_bool0[i]) {
            PRINTF("LED[%d]: ON\n", i);
        } else {
            PRINTF("LED[%d]: OFF\n", i);
...
// list
bool checkCommand(const int8_t *buffer, const char *command, size_t length) {
    if (!buffer || !command) {
        return false;
    }

    size_t i = 0;
    while (i < length && command[i] != '\0') {
        if ((char)buffer[i] != command[i]) {
            return false;
        }
        i++;
    }
...
void task_socket_srv( void *tp_arg )
{
	...
	char l_rx_buf_list[SOCKET_SRV_BUF_SIZE];
	...
	for( ;; )
	{
		...
		for ( l_reply_count = 0; pdTRUE; l_reply_count++ )
		{
			...
			if( l_len > 0 )
			{
				...
				if (checkCommand(l_rx_buf, "LIST\n", 4)) {
				    char listResponse[712];
				    vTaskList(listResponse);

				    for (char *c = listResponse; *c != '\0'; c++) {
				        if (*c == '\r') {
				            *c = '\n';
				        }
				    }

				    l_len = FreeRTOS_send(l_sock_client, (void *)listResponse, strlen(listResponse), 0);
				}
...
void task_print_buttons( void *tp_arg ) {
	...
	while(1) {
		char msg[64] = "Stav buttonu: ";
		for(int i = 0; i < BUT_NUM; i++) {
			sprintf(msg, "%s%d", msg, but_bool[i].state ? 1 : 0);
			if(!send_message) {
				send_message = but_bool[i].change;
			}
			but_bool[i].change = false;
		}
		sprintf(msg, "%s\n", msg);
		...
		if(send_message) {
			FreeRTOS_send( l_sock_client, ( void * ) msg, 64, 0 );
			send_message = false;
		}
...
int main(void) {
    ...
    //
	uint8_t ucMAC[ ipMAC_ADDRESS_LENGTH_BYTES ] = { 0x5A, 0xFE, 0xC0, 0xDE, 142, 92 };
