...
{   
    ...
    while ( 1 )
    {
        ...
        int poll_result = poll( l_read_poll, 2, 2000 );

        //timeouted
        if ( poll_result == 0 )
        {            
            char example[32];
            generate_random_example(example, sizeof(example));
            
            int l_len = write( l_sock_server, example, strlen(example) );
            if ( l_len < 0 )
            {
                log_msg( LOG_ERROR, "Unable to send generated example to server." );
                break;
            }
            else
            {
                log_msg( LOG_INFO, "Sent generated example: %s", example );
            }
            continue;
        }
