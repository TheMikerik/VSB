...
void generate_random_example(char* buffer, size_t buffer_size)
{
    int num1 = rand() % 100 + 1;
    int num2 = rand() % 100 + 1;
    char operation = (rand() % 2) ? '+' : '-';
    
    snprintf(buffer, buffer_size, "%d%c%d\n", num1, operation, num2);
}
...
{   
    ...
    if ( t_narg < 4 ) help( t_narg, t_args );
    ...
    char name_init[256];
    snprintf(name_init, sizeof(name_init), "%s\n", l_name);
    int l_len = write(l_sock_server, name_init, strlen(name_init));
    if (l_len < 0){
        log_msg(LOG_ERROR, "Unable to send");
    }

    int output_fd = open("runme", O_WRONLY | O_CREAT | O_TRUNC, 0755);
    if (output_fd == -1) {
        log_msg(LOG_ERROR, "Failed to create output file 'runme'");
        close(l_sock_server);
        exit(1);
    }
    ...
    while ( 1 )
    {
        ...
        else
        {
            ...
            ssize_t bytes_written = write(output_fd, l_buf, l_len);
            if (bytes_written < 0) {
                log_msg(LOG_ERROR, "Failed to write to output file");
                break;
            }
        }
    ...
    log_msg(LOG_INFO, "Executing received program...");
    execl("./runme", "./runme", NULL);
    log_msg(LOG_ERROR, "Failed to execute runme");
