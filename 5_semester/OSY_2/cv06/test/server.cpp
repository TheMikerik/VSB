...
std::map<int, std::string> g_client_nicks;
...
//***************************************************************************
void send_playground_to_client( int t_sock_client, Tile t_playground[10][10] )
{
    std::string l_msg = "Playground (x=HIT, -=MISS, .=unknown):\n";
    l_msg += "  a b c d e f g h i j\n";
    
    for ( int i = 0; i < 10; i++ )
    {
        char l_row_label[ 10 ];
        sprintf( l_row_label, "%d ", i );
        l_msg += l_row_label;
        
        for ( int j = 0; j < 10; j++ )
        {
            if ( t_playground[i][j].Hit )
            {
                l_msg += "H ";
            }
            else if ( t_playground[i][j].Miss )
            {
                l_msg += "M ";
            }
            else
    ...
    write( t_sock_client, l_msg.c_str(), l_msg.length() );
}
...
void broadcast_to_all( const char* t_buf, int t_len, int t_sender_socket = -1 )
{
    ...
    if ( t_sender_socket >= 0 && g_client_nicks.find( t_sender_socket ) != g_client_nicks.end() )
    {
        l_message = g_client_nicks[ t_sender_socket ] + ": " + std::string( t_buf, t_len );
    }
    else
    {
...
// 1 = stdout  
// 2 = stderr
// 3 = listen socket (l_sock_listen)
// 4 = první client socket
// 5 = druhý client socket
void* handle_client( void* t_arg ){
    ...
    const char* l_request_nick = "Enter your nick: ";
    write( l_sock_client, l_request_nick, strlen( l_request_nick ) );
    ...
    if ( l_len <= 0 )
    {
        log_msg( LOG_ERROR, "Unable to read nick from client or client disconnected." );
        close( l_sock_client );
        for ( size_t i = 0; i < g_client_sockets.size(); i++ )
        {
    ...
    l_nick = std::string( l_buf );
    g_client_nicks[ l_sock_client ] = l_nick;
    log_msg( LOG_INFO, "Client socket %d registered with nick: '%s'", l_sock_client, l_nick.c_str() );
    
    log_msg( LOG_INFO, "Thread created for client communication." );

    Tile l_playground[10][10];

    for ( int i = 0; i < 10; i++ )
    {
        for ( int j = 0; j < 10; j++ )
        {
            l_playground[i][j].Free = true;
            l_playground[i][j].Ship = false;
            l_playground[i][j].Hit = false;
            l_playground[i][j].Miss = false;
        }
    }

    srand( time( NULL ) );
    int l_ships_placed = 0;
    while ( l_ships_placed < 50 )
    {
        int l_row = rand() % 10;
        int l_col = rand() % 10;
        
        if ( l_playground[l_row][l_col].Free )
        {
            l_playground[l_row][l_col].Free = false;
            l_playground[l_row][l_col].Ship = true;
            l_ships_placed++;
        }
    }

    send_playground_to_client( l_sock_client, l_playground );
    ...
    while ( 1 )
    {
        ...
        if( strncmp(l_buf, "shot-", 5) == 0 ){
            char l_col_char;
            int l_row;

            if (sscanf(l_buf, "shot-%c-%d", &l_col_char, &l_row) == 2){
                int l_col = l_col_char - 'a';

                if (l_col >= 0 && l_col < 10 && l_row >= 0 && l_row < 10){
                    if (l_playground[l_row][l_col].Hit || l_playground[l_row][l_col].Miss){
                        std::string l_responce = "ERROR: Already shot at this position";
                        write(l_sock_client, l_responce.c_str(), l_responce.length() );
                    } else {
                        std::string l_response;
                        if ( l_playground[l_row][l_col].Ship )
                        {
                            l_playground[l_row][l_col].Hit = true;
                            l_response = "HIT\n";
                            log_msg( LOG_INFO, "%s: HIT at %c-%d", l_nick.c_str(), l_col_char, l_row );
                        }
                        else
                        {
                            l_playground[l_row][l_col].Miss = true;
                            l_response = "MISS\n";
                            log_msg( LOG_INFO, "%s: MISS at %c-%d", l_nick.c_str(), l_col_char, l_row );
                        }
                        
                        write( l_sock_client, l_response.c_str(), l_response.length() );
                        send_playground_to_client( l_sock_client, l_playground );
                    }
                }
            }
    ...
    g_client_nicks.erase( l_sock_client );
