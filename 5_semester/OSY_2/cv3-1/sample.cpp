...
#define TASK_NAME_CONTROL_BRIGHTNESS "control brightness"
#define TASK_NAME_CONTROL_SEMAPHOR_BRIGHTNESS "control semaphor"
...
float brightness_semaphor[LED_PTA_NUM] = {
	0.f,
	0.f,
};
...
void default_led_state()
{
	...
	brightness_semaphor[0] = 0.f;
	vTaskDelay(200);
	brightness_semaphor[1] = 1.f;
	vTaskDelay(200);
	brightness_semaphor[1] = 0.f;
}
void lights_down()
{
...
void task_led_pta_blink(void *t_arg)
{
	...
	while (1)
	{
		...
		brightness_semaphor[l_inx] = led_brightness;
		led_brightness += increment;
		vTaskDelay(100);
		if (led_brightness >= 1.0f)
		{
			increment *= -1;
		}
		if (led_brightness <= 0)
		{
			brightness_semaphor[l_inx] = 0;
			increment *= -1;
			l_inx++;
			l_inx %= LED_PTA_NUM;
		}
...
void task_control_brightness(void *t_args)
{
	...
	while (1)
	{
		for (int inx = 0; inx < LED_PTC_NUM; inx++)
		{
			if (brightness[inx] > 0)
			{
				int helper = (int)(brightness[inx] * max_counter);
				if (helper > counter)
				{
...
void task_control_semaphor_brightness(void *t_args)
{
	const int max_counter = 20;
	int counter = 0;
	while (1)
	{
		...
		{
			if (brightness_semaphor[inx] > 0)
			{
				int helper = (int)(brightness_semaphor[inx] * max_counter);
				if (helper > counter)
				{
...
bool semafor_on = true;
bool left_train_arrived = false;
bool right_train_arrived = false;
...
void task_snake_left(void *t_arg)
{
	...
	{
		vTaskSuspend(0);
		if (right_train_arrived || !semafor_on)
		{
			continue;
		}

		if (left_train_arrived)
		{
			...
			left_train_arrived = false;
		}
		else
		{
			for (int inx = 0; inx < LED_PTC_NUM; inx++)
			{
				left_train_arrived = true;
				// switch LED on
				brightness[inx] = 1.f;
				if (inx > 0)
					brightness[inx - 1] = 0.05f;
				vTaskDelay(200);
			}
		}
	}
...
void task_snake_right(void *t_arg)
{
	...
	while (1)
	{
		...
		if (left_train_arrived || !semafor_on)
		{
			continue;
		}

		if (right_train_arrived)
		{
			...
			right_train_arrived = false;
		}
		else
		{
			right_train_arrived = true;
			for (int inx = LED_PTC_NUM - 1; inx >= 0; inx--)
			{
				// switch LED on
				brightness[inx] = 1.f;
...
void task_switches(void *t_arg)
{
	...
	{
		// test PTC9 switch
		if (GPIO_PinRead(SW_PTC9_GPIO, SW_PTC9_PIN) == 0 && !hold)
		{
			if (l_handle_led_pta)
		...
		// test PTC10 switch
		if (GPIO_PinRead(SW_PTC10_GPIO, SW_PTC10_PIN) == 0 && !hold)
		{
			if (l_handle_led_pta)
		...
		// PTC9 and PTC10 long hold
		if (GPIO_PinRead(SW_PTC9_GPIO, SW_PTC9_PIN) == 0 && GPIO_PinRead(SW_PTC10_GPIO, SW_PTC10_PIN) == 0 && !hold)
		{
			if (l_handle_led_pta)
				vTaskSuspend(l_handle_led_pta);
			lights_down();
		...
		if (GPIO_PinRead(SW_PTC9_GPIO, SW_PTC9_PIN) == 1 && GPIO_PinRead(SW_PTC10_GPIO, SW_PTC10_PIN) == 1)
		{
			hold = false;
		}
		// test PTC11 switch
...
int main(void)
{
	...
	if (xTaskCreate(task_control_brightness, TASK_NAME_CONTROL_BRIGHTNESS, configMINIMAL_STACK_SIZE + 100, NULL, NORMAL_TASK_PRIORITY, NULL) != pdPASS)
	{
		PRINTF("Unable to create task '%s'!\r\n", TASK_NAME_CONTROL_BRIGHTNESS);
	}

	if (xTaskCreate(task_control_semaphor_brightness, TASK_NAME_CONTROL_SEMAPHOR_BRIGHTNESS, configMINIMAL_STACK_SIZE + 100, NULL, NORMAL_TASK_PRIORITY, NULL) != pdPASS)
	{
		PRINTF("Unable to create task '%s'!\r\n", TASK_NAME_CONTROL_SEMAPHOR_BRIGHTNESS);
	}
