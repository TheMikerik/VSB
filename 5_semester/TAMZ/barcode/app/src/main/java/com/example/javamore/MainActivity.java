...
public class MainActivity extends AppCompatActivity {
    ...
    private Button showBarcodeButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        showBarcodeButton = findViewById(R.id.show_barcode);
        ...
        showBarcodeButton.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void showBarcode() {
        // Get the total amount to encode in the barcode
        String totalAmountStr = totalAmountTextView.getText().toString().replaceAll("[^0-9]", "");
        int totalAmount;
        try {
            totalAmount = Integer.parseInt(totalAmountStr);
        } catch (NumberFormatException e) {
            totalAmount = 7777; // Default barcode number
        }

        // Ensure the totalAmount is a 12-digit number for UPC-A
        String totalFormatted = String.format("%012d", totalAmount % 1000000000000L); // Limit to 12 digits
        int[] barcodeDigits = new int[12];
        for (int i = 0; i < 12; i++) {
            barcodeDigits[i] = Character.getNumericValue(totalFormatted.charAt(i));
        }

        // Set the code in BarcodeView
        barcodeView.setCode(barcodeDigits);

        // Make the BarcodeView visible
        barcodeView.setVisibility(View.VISIBLE);

        // Optionally, hide the BarcodeView when clicked
