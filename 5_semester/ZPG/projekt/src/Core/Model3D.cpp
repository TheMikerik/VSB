...
Model3D::Model3D(const std::string& path,
               const std::shared_ptr<ShaderProgram>& externalShader)
    : shader(externalShader) // Use the external shader if provided
{
    if(!shader) { // If no external shader is provided, initialize the default shader
...
void Model3D::render(const glm::mat4& view, const glm::mat4& projection, const glm::mat4& modelMatrix) {
    if(!shader) {
        std::cerr << "Shader program not initialized for Model3D." << std::endl;
        return;
    }
...
std::vector<std::shared_ptr<Texture>> Model3D::loadMaterialTextures(aiMaterial* mat, aiTextureType type, const std::string& typeName) {
    std::vector<std::shared_ptr<Texture>> textures;
