...
void Controller::processKeyboardInput(float deltaTime) {
    ...
    static bool sceneKeyPressedLastFrame[8] = {false, false, false, false, false, false, false, false};
    ...
    bool sceneKey7PressedThisFrame = glfwGetKey(window, GLFW_KEY_7) == GLFW_PRESS;
    bool sceneKey8PressedThisFrame = glfwGetKey(window, GLFW_KEY_8) == GLFW_PRESS;
    ...
    if (sceneKey7PressedThisFrame && !sceneKeyPressedLastFrame[6]) switchScene(6);
    if (sceneKey8PressedThisFrame && !sceneKeyPressedLastFrame[7]) switchScene(7);
    ...
    sceneKeyPressedLastFrame[5] = sceneKey6PressedThisFrame;
    sceneKeyPressedLastFrame[6] = sceneKey7PressedThisFrame;
    sceneKeyPressedLastFrame[7] = sceneKey8PressedThisFrame;
    ...
    if (fPressedThisFrame && !fPressedLastFrame) {
        if (currentSceneIndex >= 0 && currentSceneIndex < static_cast<int>(scenes.size())) {
            ...
            auto scene_arcade = std::dynamic_pointer_cast<Scene_arcade>(scenes[currentSceneIndex]);
            auto scene_bezier = std::dynamic_pointer_cast<Scene_bezier>(scenes[currentSceneIndex]);
            if (scene6) {
                scene6->toggleFlashlight();
            }
            if (scene_arcade) {
                scene_arcade->toggleFlashlight();
            }
            if (scene_bezier) {
                scene_bezier->toggleFlashlight();
            }
        }
    }
