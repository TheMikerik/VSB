...
class CodeGenerator extends MyLangBaseVisitor<void> {
  ...
  @override
  void visitAppendExpr(AppendExprContext ctx) {
    final exprs = ctx.unarys();

    if (exprs.length > 1) {
      visit(exprs[0]);

      String? fileVar;
      var fileVarExpr = exprs[0];
      fileVar = fileVarExpr.text;

      for (int i = 1; i < exprs.length; i++) {
        _output.writeln('load $fileVar');
        visit(exprs[i]);
        _output.writeln('fappend');
      }

      _lastExpressionType = null;
    } else {
      visit(exprs[0]);
    }
  }
  ...
  void visitMultiplicative(MultiplicativeContext ctx) {
    if (ctx.appendExprs().length > 1) {
      visit(ctx.appendExpr(0)!); // Visit first operand
      VarType currentType = _lastExpressionType!;

      for (int i = 1; i < ctx.appendExprs().length; i++) {
        final op = ctx.getChild(i * 2 - 1)!.text; // *, /, %
        visit(ctx.appendExpr(i)!); // Visit next operand
        final rightType = _lastExpressionType!;
      ...
      visit(ctx.appendExpr(0)!); // Only one operand
    }
  }
