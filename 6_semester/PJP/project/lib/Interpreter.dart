...
class Interpreter {
  ...
  // Run the interpreter on a file of instructions
  void run(String filename) {
    final lines = File(filename).readAsLinesSync();
    final instructions = _preprocess(lines);

    _ic = 0;
    while (_ic < instructions.length) {
      final instruction = instructions[_ic];
      _executeInstruction(instruction);
      _ic++;
    }
  }

  // Preprocess instructions to find labels
    ...
    switch (op) {
      ...
      case 'jmp':
        ...
        if (_labels.containsKey(labelNum)) {
          _ic =
              _labels[labelNum]! - 1; // -1 because we increment after execution
        } else {
  ...
  // // Handle comparison operations
  // void _handleComparison(String op, String type) {
  //   final b = _stack.removeFirst();
  //   final a = _stack.removeFirst();
