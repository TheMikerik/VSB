    ...
    public:
        ...
        void ApplyInterest(){
            for (int i=0; i<Account::GetAccountCount(); i++){
                accounts[i]->Deposit( accounts[i]->GetBalance() * ( accounts[i]->GetInterestRate() / 100 ) );
            }
        }
        ...
        void DeleteAccount(int id){
            ...
            if (temp->GetBalance() != 0){
                cout << "This account cannot be delted." << endl;
                if (temp->GetBalance() > 0){
                    cout << "Account is not empty. It has " << temp->GetBalance() << " czk deposited.\nWithdraw it first please." << endl;
                }
                else if (temp->GetBalance() < 0){
                    cout << "Account has negative balance of " << temp->GetBalance() << " czk\nOwner have to pay the debt first. "<< endl;
                }
            }
            else{
            cout << "     Account with id " << temp->GetNumber() << " has been delted." << endl;
            delete temp;
            }
        };
};