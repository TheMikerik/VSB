...
int main(){
    ...
    testBank.GetAccount(0)->Deposit( 5500 );
    testBank.GetAccount(1)->Deposit( 10500 );
    testBank.GetAccount(3)->Deposit( 7777 );
    ...
    cout << "\nNow lets add interest into all accounts\n";
    for (int i=0; i<Account::GetAccountCount();i++){
        cout << "     Account_" << testBank.GetAccount(i)->GetNumber() <<
        " had balance of: " << testBank.GetAccount(i)->GetBalance() << " czk" << endl;
    }
    cout << "\nInterest has been applied\n";
    testBank.ApplyInterest();
    for (int i=0; i<Account::GetAccountCount();i++){
        cout << "     Account_" << testBank.GetAccount(i)->GetNumber() <<
        " has balance of: " << testBank.GetAccount(i)->GetBalance() << " czk" << endl;
    }




    cout << "\nNow lets delete account number 2\n";
    testBank.DeleteAccount(1);
